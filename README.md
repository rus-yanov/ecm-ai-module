...
| PAYMENT | 4 | 50% | 2 low_ocr_confidence |
| **ИТОГО** | **15** | **33%** | — |

**Производительность:** среднее время обработки 57 с на сканированный документ (CPU).
...
- PAYMENT частично верифицирован на сканах: 2/4 верно; атрибуты (сумма, ИНН) плохо извлекаются из OCR-текста платёжных форм
- Производительность: text fast-path ≈ 34 с, OCR-скан ≈ 57 с (CPU); GPU даст 5–10× ускорение
